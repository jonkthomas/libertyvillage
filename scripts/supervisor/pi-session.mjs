...
export function buildGeneratorPrompt({ topic, contextFiles = [], now = new Date() } = {}) {
  ...
  return [
    ...
    'Read data/businesses.json before naming any business. A bare display name can match more than one record (two rows are both named Cibo Wine Bar; only cibo-liberty-village contains happy hour). Identify a venue only as a markdown directory link with that record\'s exact unique slug: [Name](/directory/<slug>). Never write the bare shared name.',
    'Attribute an operational policy (happy hour, pet-friendly, reservations, accessibility) to a venue only when THAT SAME slug\'s record contains the policy text. If the topic is happy hour, only link slugs whose records contain "happy hour"; do not mention King West Cibo or any namesake that lacks the policy. If you cannot name a supporting venue without also attributing an unsupported namesake, write a neighbourhood guide that names no businesses and does not put happy hour, pet-friendly, reservations, or accessibility in the slug or title.',
    'Do not write clock ranges, a.m./p.m. times, dollar amounts, civic addresses, or opening hours. Happy-hour times and prices live in proTip/answerBlock, not the hours/address/priceRange fields the linter checks, so quoting them fails even when the record mentions them.',
