...
export function buildGeneratorPrompt({ topic, contextFiles = [], businesses = topic?.businesses ?? [], now = new Date() } = {}) {
  ...
  return [
    ...
    'Local specifics are required when they are copied verbatim from the linked repository record for that business: address, price, and hours claims may appear only as exact verbatim copies of that record\'s own text, throughout title, slug, description, content, answerBlock, FAQs, and keyTakeaways. A specific the record does not contain must be omitted — never approximated, never written from memory, never carried over from another record.',
    'Do not invent or approximate any clock range, dollar amount, civic address, or opening hours anywhere in the post, including keyTakeaways and FAQs. Copy each specific verbatim from the record that owns it, or leave it out and write a correct, vaguer sentence. Bare business-name claims remain forbidden; every business is a /directory/ link.',
